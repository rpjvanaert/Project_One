...
public class ToeLogic {
    ...
    public boolean placePos(Point2D place){
        if (toes[(int)place.getX()][(int)place.getY()] == 0){
            toes[(int)place.getX()][(int)place.getY()] = 1;
            return true;
        }
        return false;
    }
    ...
    public int checkForWin(){
        ArrayList<Integer> checks = new ArrayList<>(Arrays.asList(this.checkX(), this.checkY()));
    ...
    private int checkX(){
        ...
        for (int x = 0; x < width - 3; ++x){
            for (int y = 0; y < height; ++y){
                value = this.checkPlaces(new Point2D.Double(x, y), new Point2D.Double(x + 1, y), new Point2D.Double(x + 2, y), new Point2D.Double(x + 3, y));
                if (value == 1){
                    return 1;
                } else if (value == 2){
                    return 2;
    ...
    private int checkY(){
        ...
        for (int y = 0; y < height - 3; ++y){
            for (int x = 0; x < width; ++x){
                value = this.checkPlaces(new Point2D.Double(x, y), new Point2D.Double(x, y + 1), new Point2D.Double(x, y + 2), new Point2D.Double(x, y + 3));
                if (value == 1){
                    return 1;
                } else if (value == 2){
                    return 2;
    ...
    private int checkPlaces(Point2D p1, Point2D p2, Point2D p3, Point2D p4){
